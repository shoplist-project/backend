...
def create_shop_list():
    ...
    if not shop_list:
        return jsonify({"error": "Failed to create shop list"}), 500
...
def share_shop_list(shop_list_id):
    ...
    if not shop_list:
        return jsonify({"error": "Shop list not found"}), 404

    if shop_list.owner_id != user_id:
        return jsonify({"error": "Only the owner can share this shop list"}), 403

    data = request.get_json()

    if not data or "users" not in data or "access" not in data:
        return jsonify({"error": "Missing required fields: users, access"}), 400

    usernames = data["users"]

    if not isinstance(usernames, list):
        return jsonify({"error": "Field users must be an array of usernames"}), 400
    ...
    shared_user_ids = ShopListService.share_shop_list(shop_list_id, usernames, access)

    if shared_user_ids is None:
        return jsonify({"error": "Failed to share shop list"}), 500
...
def unshare_shop_list(shop_list_id):
    ...
    unshared_user_ids = ShopListService.unshare_shop_list(shop_list_id, usernames)

    if unshared_user_ids is None:
        return jsonify({"error": "Failed to unshare shop list"}), 500
...
def delete_shop_list(shop_list_id):
    ...
    if shop_list.owner_id != user_id:
        return jsonify({"error": "Only the owner can delete this shop list"}), 403

    success = ShopListService.delete_shop_list(shop_list_id)

    if not success:
        return jsonify({"error": "Failed to delete shop list"}), 500
...
def update_shop_list(shop_list_id):
    ...
    if shop_list.owner_id != user_id:
        return jsonify({"error": "Only the owner can edit this shop list"}), 403
    ...
    if not name or not name.strip():
        return jsonify({"error": "Shop list name cannot be empty"}), 400

    updated_shop_list = ShopListService.update_shop_list(shop_list_id, name)

    if not updated_shop_list:
        return jsonify({"error": "Failed to update shop list"}), 500
