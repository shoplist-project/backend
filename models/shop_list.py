...
class ShopList(db.Model):
    ...
    products = db.relationship('Product', backref='shop_list', lazy=True, cascade='all, delete-orphan')
    shared_with = db.relationship('ShopListShare', backref='shop_list', lazy=True, cascade='all, delete-orphan')
...
class Product(db.Model):
    ...
    shop_list_id = db.Column(db.String(36), db.ForeignKey('shop_lists.id'), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
